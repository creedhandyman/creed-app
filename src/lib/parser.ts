...
export async function aiParsePdf(
  ...
  try {
    ...
    let cleanText = text;
    if (cleanText.trim()) {
      // Remove "Page X of Y" and "Report generated by zInspector" noise
      ...
      // Try to strip the summary table (pages before detailed breakdowns)
      // The detailed section starts when room names appear as section headers
      // followed by "Detail Condition Actions Comment" column headers
      const detailStart = cleanText.search(/\n(Kitchen|Appliances|Living Room|Laundry|Dining|Entry|Hallway|Bedroom|Bathroom|Exterior|Compliance)\s*\n\s*Detail\s+Condition/i);
      if (detailStart > 200) {
        // Found the start of detailed breakdowns — strip everything before it
        cleanText = cleanText.slice(detailStart);
      } else {
        // Alternative: strip the "Move Out Condition Summary" table
        // Look for the "Area Detail Condition Actions Comment Media" header pattern
        const summaryEnd = cleanText.search(/\n(Kitchen|Appliances|Living Room|Laundry|Dining|Entry|Hallway|Bedroom|Bathroom|Exterior|Compliance)\s*\nDetail\s+Condition\s+Actions/i);
        if (summaryEnd > 200) {
          cleanText = cleanText.slice(summaryEnd);
        }
      }

      // Remove duplicate lines (same line appearing within 50 characters)
      const lines = cleanText.split("\n");
      const seen = new Set<string>();
      const deduped: string[] = [];
      for (const line of lines) {
        const key = line.trim().toLowerCase().slice(0, 60);
      ...
      content.push({
        type: "text",
        text: `Here is the DETAILED ROOM BREAKDOWN section from the inspection report (summary table already removed):\n\n${cleanText.slice(0, 30000)}`,
      });
    }
