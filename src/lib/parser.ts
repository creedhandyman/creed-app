...
export function validateQuote(rooms: Room[], opts?: { skipCaps?: boolean }): Room[] {
  ...
  if (rooms.length > 0) {
    ...
    const overrideTrade = (item: RoomItem): string | null => {
      ...
      // windows (new, full replacement)" landed in PAINTING because the
      // AI followed the prompt's caulk-is-Painting rule and the scorer's
      // Carpentry +10 on "window" couldn't beat Painting +10 on "paint"
      // + extra spackle/etc. points. Goes BEFORE the caulk override so
      // window/door perimeter caulk doesn't get pulled into Painting.
      // Replacing a knob, hinge, lock, frame hardware, etc. is already
      // Carpentry via the regular scorer; no carve-out needed.
      if (
        /\breplace\b[^.]{0,40}\bwindows?\b/.test(s) ||
        /\bwindows?\b[^.]{0,40}\breplacement\b/.test(s) ||
        /\binstall\b[^.]{0,40}\b(new\s+)?windows?\b/.test(s) ||
        /\breplace\b[^.]{0,40}\bdoors?\b/.test(s) ||
        /\bdoors?\b[^.]{0,40}\breplacement\b/.test(s) ||
        /\binstall\b[^.]{0,40}\b(new\s+)?doors?\b/.test(s)
      ) {
        return "Carpentry";
      }
...
async function aiParsePdfSingle(
  ...
  try {
    ...
    if (cleanText.trim()) {
      ...
      // ── Multi-unit detection (duplex / multi-unit reports) ──
      // The slicing + dedup below were built for SINGLE-unit reports and
      // destroy duplexes: unit B's near-identical lines get deduped away,
      // the summary-slice cuts both units' address headers, and the 30k cap
      // can truncate unit B entirely — so its work orders end up missing or
      // mislabeled under unit A. Detect multi-unit by distinct house numbers
      // among address-like strings (1105 vs 1107 — re-spellings of ONE
      // address share their number) or distinct Unit/Apt markers, and skip
      // the destructive transforms for those reports.
      const addrMatches = cleanText.match(
...
- Attribute each finding to the unit whose section of the report it appears in.
- Set "property" to the shared address covering all units (e.g. "1105 & 1107 W 51st St N").

${cleanText.slice(0, 60000)}`
          : `IMPORTANT: This text is from the DETAILED ROOM BREAKDOWN section ONLY. The summary table has been removed. Each room section below should be processed ONCE. Do NOT create duplicate entries.\n\n${cleanText.slice(0, 30000)}`,
      });
    }
...
export function estimateMaterials(t: string): Material[] {
  ...
  // Legacy inline matchers were consolidated into MATERIALS_DB (materials-db.ts);
  // the curated loop above now covers them.
